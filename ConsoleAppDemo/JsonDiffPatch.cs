    ...
    public class JsonDiffPatch
    {
        ...
        public JToken Diff(JToken left, JToken right)
        {
            ...
            if (this._options.TextDiff == TextDiffMode.Efficient && left.Type == JTokenType.String && right.Type == JTokenType.String && ((long)left.ToString().Length > this._options.MinEfficientTextDiffLength || (long)right.ToString().Length > this._options.MinEfficientTextDiffLength))
            {
                ...
                List<DiffMatchPatch.Patch> patchList = diffMatchPatch.patch_make(left.ToObject<string>(), right.ToObject<string>());
                if (!patchList.Any<DiffMatchPatch.Patch>())
                    return (JToken)null;
            ...
            switch (jarray[2].Value<int>())
            {
                ...
                case 2:
                    if (left.Type != JTokenType.String)
                        throw new InvalidDataException("Invalid patch object");
                    diff_match_patch diffMatchPatch = new diff_match_patch();
                    List<DiffMatchPatch.Patch> patchList = diffMatchPatch.patch_fromText(jarray[0].ToObject<string>());
                    if (!patchList.Any<DiffMatchPatch.Patch>())
            ...
            switch (jarray[2].Value<int>())
            {
                ...
                case 2:
                    if (right.Type != JTokenType.String)
                        throw new InvalidDataException("Invalid patch object");
                    diff_match_patch diffMatchPatch = new diff_match_patch();
                    List<DiffMatchPatch.Patch> source = diffMatchPatch.patch_fromText(jarray[0].ToObject<string>());
                    if (!source.Any<DiffMatchPatch.Patch>())
                        throw new InvalidDataException("Invalid textline");
                    List<DiffMatchPatch.Patch> patches = new List<DiffMatchPatch.Patch>();
                    for (int index = source.Count - 1; index >= 0; --index)
                    {
                        DiffMatchPatch.Patch patch2 = source[index];
                        DiffMatchPatch.Patch patch3 = new DiffMatchPatch.Patch()
                        {
                        ...
                        foreach (DiffMatchPatch.Diff diff in patch2.diffs)
                        {
                            if (diff.operation == Operation.DELETE)
                                patch3.diffs.Add(new DiffMatchPatch.Diff(Operation.INSERT, diff.text));
                            else if (diff.operation == Operation.INSERT)
                                patch3.diffs.Add(new DiffMatchPatch.Diff(Operation.DELETE, diff.text));
                            else
